...
// RocksDBEtcdRaft integrates Raft consensus with etcd-compatible RocksDB storage
type RocksDBEtcdRaft struct {
	db          *grocksdb.DB
...
// NewRocksDBEtcdRaft creates a new RocksDB + Raft + etcd semantic storage
func NewRocksDBEtcdRaft(
	db *grocksdb.DB,
	snapshotter *snap.Snapshotter,
	proposeC chan<- string,
...
func (r *RocksDBEtcdRaft) readCommits(commitC <-chan *kvstore.Commit, errorC <-chan error) {
...
func (r *RocksDBEtcdRaft) applyOperation(op RaftOperation) {
	switch op.Type {
	case "PUT":
		// Apply PUT
...
func (r *RocksDBEtcdRaft) applyLegacyOp(data string) {
...
func (r *RocksDBEtcdRaft) CurrentRevision() int64 {
...
func (r *RocksDBEtcdRaft) incrementRevision() (int64, error) {
...
func (r *RocksDBEtcdRaft) Range(key, rangeEnd string, limit int64, revision int64) (*kvstore.RangeResponse, error) {
...
func (r *RocksDBEtcdRaft) PutWithLease(key, value string, leaseID int64) (int64, *kvstore.KeyValue, error) {
...
func (r *RocksDBEtcdRaft) DeleteRange(key, rangeEnd string) (int64, []*kvstore.KeyValue, int64, error) {
...
func (r *RocksDBEtcdRaft) deleteUnlocked(key, rangeEnd string) error {
...
func (r *RocksDBEtcdRaft) LeaseGrant(id int64, ttl int64) (*kvstore.Lease, error) {
	// Generate sequence number
	r.mu.Lock()
	r.seqNum++
	seqNum := fmt.Sprintf("seq-%d", r.seqNum)
...
func (r *RocksDBEtcdRaft) leaseGrantUnlocked(id int64, ttl int64) error {
	lease := &kvstore.Lease{
		ID:        id,
		TTL:       ttl,
		GrantTime: timeNow(),
...
func (r *RocksDBEtcdRaft) LeaseRevoke(id int64) error {
	// Generate sequence number
	r.mu.Lock()
	r.seqNum++
	seqNum := fmt.Sprintf("seq-%d", r.seqNum)
...
func (r *RocksDBEtcdRaft) Watch(key, rangeEnd string, startRevision int64, watchID int64) (<-chan kvstore.WatchEvent, error) {
	return r.WatchWithOptions(key, rangeEnd, startRevision, watchID, nil)
}

// WatchWithOptions creates a watch with options
func (r *RocksDBEtcdRaft) WatchWithOptions(key, rangeEnd string, startRevision int64, watchID int64, opts *kvstore.WatchOptions) (<-chan kvstore.WatchEvent, error) {
	r.watchMu.Lock()
...
func (r *RocksDBEtcdRaft) Compact(revision int64) error {
...
func (r *RocksDBEtcdRaft) LeaseRenew(id int64) (*kvstore.Lease, error) {
	// Get current lease
	lease, err := r.getLease(id)
	if err != nil {
		return nil, err
...
func (r *RocksDBEtcdRaft) LeaseTimeToLive(id int64) (*kvstore.Lease, error) {
	return r.getLease(id)
}

// Leases returns all leases
func (r *RocksDBEtcdRaft) Leases() ([]*kvstore.Lease, error) {
...
func (r *RocksDBEtcdRaft) Propose(k string, v string) {
	// Convert to etcd Put operation
	r.PutWithLease(k, v, 0)
}

// Lookup looks up a key (for backward compatibility)
func (r *RocksDBEtcdRaft) Lookup(key string) (string, bool) {
	kv, err := r.getKeyValue(key)
	if err != nil || kv == nil {
		return "", false
	}
...
func (r *RocksDBEtcdRaft) Txn(compare []kvstore.Compare, onSuccess []kvstore.Op, onFailure []kvstore.Op) (*kvstore.TxnResponse, error) {
...
func (r *RocksDBEtcdRaft) getKeyValue(key string) (*kvstore.KeyValue, error) {
	dbKey := []byte(kvPrefix + key)
...
func (r *RocksDBEtcdRaft) getLease(id int64) (*kvstore.Lease, error) {
	dbKey := []byte(fmt.Sprintf("%s%d", leasePrefix, id))
	data, err := r.db.Get(r.ro, dbKey)
	if err != nil {
		return nil, err
...
func (r *RocksDBEtcdRaft) GetSnapshot() ([]byte, error) {
...
func (r *RocksDBEtcdRaft) loadSnapshot() (*raftpb.Snapshot, error) {
...
func (r *RocksDBEtcdRaft) recoverFromSnapshot(snapshot []byte) error {
...
func (r *RocksDBEtcdRaft) shouldFilter(eventType kvstore.EventType, filters []kvstore.WatchFilterType) bool {
...
func (r *RocksDBEtcdRaft) slowSendEvent(sub *watchSubscription, event kvstore.WatchEvent) {
...
func (r *RocksDBEtcdRaft) matchWatch(key, watchKey, rangeEnd string) bool {
	if rangeEnd == "" {
		// Single key match
		return key == watchKey
	}
