...
// Helper: create a test PebbleDB store
func createTestStore(t *testing.T, tmpDir string) (*PebbleDB, func()) {
	db, err := Open(tmpDir)
	...
	store := NewPebbleDB(db, snapshotter, proposeC, commitC, errorC)
...
func TestPebbleDB_Compact_Basic(t *testing.T) {
...
func TestPebbleDB_Compact_Validation(t *testing.T) {
...
func TestPebbleDB_Compact_ExpiredLeases(t *testing.T) {
...
func TestPebbleDB_Compact_PhysicalCompaction(t *testing.T) {
	...
	// Compact - this should trigger PebbleDB physical compaction
	err := store.Compact(context.Background(), currentRev-100)
	require.NoError(t, err)
...
func TestPebbleDB_Compact_Sequential(t *testing.T) {
...
func saveLeaseForTest(r *PebbleDB, lease *kvstore.Lease) error {
	...
	return r.db.Set(dbKey, buf.Bytes(), r.wo)
}

// Helper: get lease for testing
func getLeaseForTest(r *PebbleDB, id int64) (*kvstore.Lease, error) {
	dbKey := []byte(fmt.Sprintf("%s%d", leasePrefix, id))
	value, closer, err := r.db.Get(dbKey)
	if err != nil {
	...
	if err := gob.NewDecoder(bytes.NewReader(value)).Decode(&lease); err != nil {
		return nil, err
	}
