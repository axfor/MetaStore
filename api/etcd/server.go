...
type Server struct {
	...
	clientURLs   []string // Configured client URLs for this member
}
...
type ServerConfig struct {
	...
	ClientURLs   []string                 // Configured advertised client URLs for this member
...
func NewServer(cfg ServerConfig) (*Server, error) {
	...
	if cfg.ConfChangeC != nil {
		...
		for i, peerURL := range cfg.ClusterPeers {
			...
			// IMPORTANT:
			// Do NOT seed per-member ClientURLs here (even for self).
			// Each node must self-publish its configured client URLs via Raft so every
			// node converges on the same committed member metadata.
			// Seeding self ClientURLs locally prevents PublishClientURLs() from ever
			// proposing the update, which leaves other nodes stuck with placeholders.
			clientURLs := []string{}
...
func (s *Server) Start() error {
	...
	// Self-publish configured client URLs to the cluster.
	// Do this in a retry loop because early startup may not have a stable leader yet.
	reliability.SafeGo("publish-client-urls", func() {
		ticker := time.NewTicker(1 * time.Second)
		defer ticker.Stop()
		for {
			select {
			case <-s.shutdownMgr.Done():
				return
			case <-ticker.C:
				if err := s.PublishClientURLs(); err != nil {
					log.Debug("PublishClientURLs retry failed",
						log.Err(err),
						log.Component("server"))
					continue
				}

				// Stop retrying once stored state matches configured state.
				if s.clusterMgr == nil || len(s.clientURLs) == 0 {
					return
				}
				member, err := s.clusterMgr.GetMember(s.memberID)
				if err == nil && urlsEqual(member.ClientURLs, s.clientURLs) {
					return
				}
			}
		}
...
// PublishClientURLs publishes configured client URLs to the cluster (Self-Publish)
// This should be called after the node is ready (e.g., after Raft is initialized)
func (s *Server) PublishClientURLs() error {
	if s.clusterMgr == nil || len(s.clientURLs) == 0 {
		return nil // Nothing to publish
	}

	// Check if stored ClientURLs match configured ones
	member, err := s.clusterMgr.GetMember(s.memberID)
	if err != nil {
		// Member not found, cannot publish
		return err
	}

	// Compare stored vs configured
	if urlsEqual(member.ClientURLs, s.clientURLs) {
		return nil // Already up-to-date
	}

	// Propose update with our configured ClientURLs
	log.Info("Publishing configured client URLs to cluster",
		log.Uint64("member_id", s.memberID),
		zap.Strings("client_urls", s.clientURLs),
		log.Component("server"))

	return s.clusterMgr.UpdateMemberWithClientURLs(s.memberID, nil, s.clientURLs)
}

// urlsEqual compares two URL slices for equality
