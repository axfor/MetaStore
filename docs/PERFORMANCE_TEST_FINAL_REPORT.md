...
由于Pebble的磁盘I/O开销，大规模负载测试未能在20分钟内完成。
...
2. **Pebble 测试超时** ⚠️
   ...
   - 建议:
     - 增加测试超时到60分钟
     - 或减小测试规模到10,000操作
     - 优化Pebble配置（block cache, write buffer）
...
2. **test/performance_pebble_test.go** (461行)
   - Pebble性能测试
   - 4个测试场景
   - ⏱️ 全部超时
...
- Pebble测试日志: `/tmp/pebble_perf_test_results.txt`
