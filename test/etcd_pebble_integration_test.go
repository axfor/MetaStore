...
// etcdPebbleCluster represents a cluster of etcd-compatible Pebble nodes for testing
type etcdPebbleCluster struct {
	peers              []string
	...
	raftNodes          []pebbledb.RaftNode
	dbs                []*pebble.DB
	kvStores           []*pebbledb.PebbleDB
	servers            []*etcdapi.Server
	clients            []*clientv3.Client
}

// newEtcdPebbleCluster creates a cluster of n etcd-compatible Pebble nodes
func newEtcdPebbleCluster(t *testing.T, n int) *etcdPebbleCluster {
	...
	clus := &etcdPebbleCluster{
		...
		raftNodes:        make([]pebbledb.RaftNode, len(peers)),
		dbs:              make([]*pebble.DB, len(peers)),
		kvStores:         make([]*pebbledb.PebbleDB, len(peers)),
	...
	for i := range clus.peers {
		// Clean up data directory (raft.NewNode uses "data/pebble/{id}" path)
		os.RemoveAll(fmt.Sprintf("data/pebble/%d", i+1))

		// Create directory for Pebble
		dbPath := fmt.Sprintf("data/pebble/%d/kv", i+1)
		...
		var rn pebbledb.RaftNode
		clus.commitC[i], clus.errorC[i], clus.snapshotterReady[i], rn = raft.NewNodePebble(
			i+1, clus.peers, false, getSnapshot, clus.proposeC[i], clus.confChangeC[i], db, fmt.Sprintf("data/pebble/%d", i+1), cfg,
		)
		clus.raftNodes[i] = rn
	}
	...
	for i := range clus.peers {
		...
		if leaseManager != nil {
			go func(rn pebbledb.RaftNode, lm *etcdapi.LeaseManager) {
...
func (clus *etcdPebbleCluster) Close(t *testing.T) {
	t.Log("closing etcd pebble cluster...")
	...
	// Close databases
	for _, db := range clus.dbs {
		if db != nil {
			db.Close()
		}
	}

	// Clean data (raft.NewNode uses "data/pebble/{id}" path)
	for i := range clus.peers {
		os.RemoveAll(fmt.Sprintf("data/pebble/%d", i+1))
	}

	t.Log("closing etcd pebble cluster [done]")
}

// TestEtcdPebbleSingleNodeOperations tests basic single-node operations
func TestEtcdPebbleSingleNodeOperations(t *testing.T) {
	clus := newEtcdPebbleCluster(t, 1)
	defer clus.Close(t)
...
// TestEtcdPebbleClusterBasicConsistency tests basic cluster consistency
func TestEtcdPebbleClusterBasicConsistency(t *testing.T) {
	const numNodes = 3
	clus := newEtcdPebbleCluster(t, numNodes)
	defer clus.Close(t)
...
// TestEtcdPebbleClusterConcurrentOperations tests concurrent operations
func TestEtcdPebbleClusterConcurrentOperations(t *testing.T) {
	const numNodes = 3
	clus := newEtcdPebbleCluster(t, numNodes)
	defer clus.Close(t)
...
// TestEtcdPebbleClusterUpdateOperations tests update operations on same key
func TestEtcdPebbleClusterUpdateOperations(t *testing.T) {
	const numNodes = 3
	clus := newEtcdPebbleCluster(t, numNodes)
	defer clus.Close(t)
