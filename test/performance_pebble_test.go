...
// TestPebblePerformance_LargeScaleLoad tests Pebble system behavior under large-scale concurrent load
func TestPebblePerformance_LargeScaleLoad(t *testing.T) {
	// t.Skip("Skipping - test is too aggressive for single-node Pebble (50 clients cause incrementRevision bottleneck)")
	if testing.Short() {
		t.Skip("Skipping Pebble large-scale load test in short mode")
	}

	// Start test server with Pebble
	...
	t.Logf("Starting Pebble large-scale load test: %d clients, %d ops/client = %d total ops",
	...
	for i := 0; i < numClients; i++ {
		...
		go func(clientID int) {
			...
			for j := 0; j < operationsPerClient; j++ {
				key := fmt.Sprintf("/pebble-load-test/client-%d/key-%d", clientID, j)
	...
	// Report results
	t.Logf("Pebble load test completed in %v", duration)
	...
	// Assert acceptable performance (Pebble may be slower due to disk I/O)
	...
	if avgLatency > 300*time.Millisecond {
		t.Errorf("Average latency too high: %v (expected < 300ms for Pebble)", avgLatency)
	}

	if throughput < 200 {
		t.Errorf("Throughput too low: %.2f ops/sec (expected > 200 for Pebble)", throughput)
	}
}

// TestPebblePerformance_SustainedLoad tests Pebble stability under sustained load
func TestPebblePerformance_SustainedLoad(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping Pebble sustained load test in short mode")
	}
	...
	t.Logf("Starting Pebble sustained load test: %d clients for %v", numClients, duration)
	...
	for i := 0; i < numClients; i++ {
		...
		go func(clientID int) {
			...
			for ctx.Err() == nil {
				key := fmt.Sprintf("/pebble-sustained/client-%d/op-%d", clientID, opCount)
	...
	t.Logf("Pebble sustained load test completed")
...
// TestPebblePerformance_MixedWorkload tests Pebble realistic mixed workload
func TestPebblePerformance_MixedWorkload(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping Pebble mixed workload test in short mode")
	}
	...
	t.Logf("Starting Pebble mixed workload test: %d clients for %v", numClients, testDuration)
	...
	for i := 0; i < numClients*2/5; i++ {
		...
		go func(id int) {
			...
			opNum := 0
			for ctx.Err() == nil {
				key := fmt.Sprintf("/pebble-mixed/put-%d-%d", id, opNum)
	...
	for i := 0; i < numClients*2/5; i++ {
		...
		go func(id int) {
			...
			opNum := 0
			for ctx.Err() == nil {
				key := fmt.Sprintf("/pebble-mixed/put-%d-%d", id%10, opNum%100)
				_, err := cli.Get(ctx, key)
				if err != nil {
					if ctx.Err() != nil {
						return
	...
	for i := 0; i < numClients/10; i++ {
		...
		go func(id int) {
			defer wg.Done()
			for ctx.Err() == nil {
				_, err := cli.Get(ctx, "/pebble-mixed/", clientv3.WithPrefix())
				if err != nil {
					if ctx.Err() != nil {
						return
					}
	...
	t.Logf("Pebble mixed workload test completed in %v", elapsed)
...
// TestPebblePerformance_Compaction tests Pebble compaction performance
func TestPebblePerformance_Compaction(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping Pebble compaction test in short mode")
	}
	...
	t.Log("Starting Pebble compaction test")
	...
	writeStart := time.Now()
	for i := 0; i < numKeys; i++ {
		key := fmt.Sprintf("/pebble-compact/key-%05d", i)
	...
	resp, err := cli.Get(ctx, "/pebble-compact/", clientv3.WithPrefix(), clientv3.WithLimit(1))
	if err != nil {
	...
	t.Log("Pebble compaction test completed successfully")
}

// TestPebblePerformance_WatchScalability tests Pebble watch performance with many watchers
func TestPebblePerformance_WatchScalability(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping watch scalability test in short mode")
	}

	_, cli, cleanup := startTestServerPebble(t)
	defer cleanup()
	...
	t.Logf("Starting Pebble watch scalability test: %d watchers, %d events", numWatchers, numEvents)
	...
	t.Logf("Pebble watch test completed in %v", duration)
