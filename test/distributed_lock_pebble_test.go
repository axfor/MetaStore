...
// ============================================================================
// Pebble Distributed Lock Test Helper
// ============================================================================

// startPebbleLockTestServer starts a Pebble-backed server for lock testing
...
// ============================================================================
// Pebble Session Tests
// ============================================================================

func TestPebble_SessionCreate(t *testing.T) {
	cli, cleanup := startPebbleLockTestServer(t)
	defer cleanup()
...
func TestPebble_SessionWithExistingLease(t *testing.T) {
	cli, cleanup := startPebbleLockTestServer(t)
	defer cleanup()
	ctx := context.Background()
...
// ============================================================================
// Pebble Basic Mutex Tests
// ============================================================================

func TestPebble_MutexLockUnlock(t *testing.T) {
	cli, cleanup := startPebbleLockTestServer(t)
	defer cleanup()
	ctx := context.Background()
	...
	mutex := concurrency.NewMutex(session, "/pebble/test/lock")
...
func TestPebble_MutexReentrantLock(t *testing.T) {
	cli, cleanup := startPebbleLockTestServer(t)
	defer cleanup()
	ctx := context.Background()
	...
	mutex := concurrency.NewMutex(session, "/pebble/test/reentrant")
...
// ============================================================================
// Pebble TryLock Tests
// ============================================================================

func TestPebble_TryLockSuccess(t *testing.T) {
	cli, cleanup := startPebbleLockTestServer(t)
	defer cleanup()
	ctx := context.Background()
	...
	mutex := concurrency.NewMutex(session, "/pebble/test/trylock")
...
func TestPebble_TryLockFail(t *testing.T) {
	cli, cleanup := startPebbleLockTestServer(t)
	defer cleanup()
	ctx := context.Background()
	...
	mutex1 := concurrency.NewMutex(session1, "/pebble/test/trylock-fail")
	err = mutex1.Lock(ctx)
	require.NoError(t, err)
	...
	mutex2 := concurrency.NewMutex(session2, "/pebble/test/trylock-fail")
...
func TestPebble_TryLockAfterUnlock(t *testing.T) {
	cli, cleanup := startPebbleLockTestServer(t)
	defer cleanup()
	ctx := context.Background()
	...
	mutex1 := concurrency.NewMutex(session1, "/pebble/test/trylock-after-unlock")
	mutex2 := concurrency.NewMutex(session2, "/pebble/test/trylock-after-unlock")
...
// ============================================================================
// Pebble Concurrent Lock Tests
// ============================================================================

func TestPebble_MutexContention(t *testing.T) {
	cli, cleanup := startPebbleLockTestServer(t)
	defer cleanup()
	ctx := context.Background()
	...
	for i := 0; i < numClients; i++ {
		...
		go func(id int) {
			...
			mutex := concurrency.NewMutex(session, "/pebble/test/contention")

			err = mutex.Lock(ctx)
			require.NoError(t, err)

			acquired <- id
			t.Logf("Pebble Client %d acquired lock", id)
			...
			released <- id
			t.Logf("Pebble Client %d released lock", id)
		}(i)
	}
...
func TestPebble_MutexFIFOOrder(t *testing.T) {
	cli, cleanup := startPebbleLockTestServer(t)
	defer cleanup()
	ctx := context.Background()
	...
	for i := 0; i < numClients; i++ {
		...
		go func(id int) {
			...
			mutex := concurrency.NewMutex(session, "/pebble/test/fifo")
			...
			t.Logf("Pebble Client %d acquired lock at position %d", id, len(acquireOrder))
	...
	t.Logf("Pebble Acquire order: %v", acquireOrder)
...
func TestPebble_MutexCriticalSection(t *testing.T) {
	cli, cleanup := startPebbleLockTestServer(t)
	defer cleanup()
	ctx := context.Background()
	...
	for i := 0; i < numClients; i++ {
		...
		go func() {
			...
			mutex := concurrency.NewMutex(session, "/pebble/test/critical-section")
...
// ============================================================================
// Pebble Lock Timeout and Cancellation Tests
// ============================================================================

func TestPebble_MutexLockWithTimeout(t *testing.T) {
	cli, cleanup := startPebbleLockTestServer(t)
	defer cleanup()
	...
	mutex1 := concurrency.NewMutex(session1, "/pebble/test/timeout")
	...
	mutex2 := concurrency.NewMutex(session2, "/pebble/test/timeout")
...
func TestPebble_MutexLockCancellation(t *testing.T) {
	cli, cleanup := startPebbleLockTestServer(t)
	defer cleanup()
	...
	mutex1 := concurrency.NewMutex(session1, "/pebble/test/cancel")
	err = mutex1.Lock(context.Background())
	require.NoError(t, err)
	...
	mutex2 := concurrency.NewMutex(session2, "/pebble/test/cancel")
...
// ============================================================================
// Pebble Session Failure Tests
// ============================================================================

func TestPebble_MutexReleaseOnSessionClose(t *testing.T) {
	cli, cleanup := startPebbleLockTestServer(t)
	defer cleanup()
	ctx := context.Background()
	...
	mutex1 := concurrency.NewMutex(session1, "/pebble/test/session-close")
	err = mutex1.Lock(ctx)
	require.NoError(t, err)
	lockKey1 := mutex1.Key()
	...
	mutex2 := concurrency.NewMutex(session2, "/pebble/test/session-close")
	...
	select {
	case <-acquired:
		t.Log("Pebble Second session acquired lock after first session closed")
		assert.True(t, mutex2.IsOwner())
...
// ============================================================================
// Pebble Election Tests
// ============================================================================

func TestPebble_ElectionCampaign(t *testing.T) {
	cli, cleanup := startPebbleLockTestServer(t)
	defer cleanup()
	ctx := context.Background()
	...
	election := concurrency.NewElection(session, "/pebble/test/election")
...
func TestPebble_ElectionMultipleCandidates(t *testing.T) {
	cli, cleanup := startPebbleLockTestServer(t)
	defer cleanup()
	ctx := context.Background()
	...
	for i := 0; i < numCandidates; i++ {
		...
		go func(id int) {
			...
			election := concurrency.NewElection(session, "/pebble/test/multi-election")
			...
			leaderChan <- id
			t.Logf("Pebble Candidate %d became leader", id)

			time.Sleep(100 * time.Millisecond)

			election.Resign(ctx)
			t.Logf("Pebble Candidate %d resigned", id)
		}(i)
	}
...
func TestPebble_MutexHighConcurrency(t *testing.T) {
	cli, cleanup := startPebbleLockTestServer(t)
	defer cleanup()
	ctx := context.Background()
	...
	for i := 0; i < numGoroutines; i++ {
		...
		go func(id int) {
			...
			mutex := concurrency.NewMutex(session, "/pebble/test/high-concurrency")
	...
	t.Logf("Pebble Success: %d, Fail: %d", successCount, failCount)
	assert.Equal(t, int64(numGoroutines*iterations), successCount)
	assert.Equal(t, int64(0), failCount)
}

func TestPebble_MutexRapidLockUnlock(t *testing.T) {
	cli, cleanup := startPebbleLockTestServer(t)
	defer cleanup()
	ctx := context.Background()
	...
	mutex := concurrency.NewMutex(session, "/pebble/test/rapid")
...
// ============================================================================
// Pebble Compatibility Tests
// ============================================================================

func TestPebble_CompatibilityWithEtcdConcurrency(t *testing.T) {
	cli, cleanup := startPebbleLockTestServer(t)
	defer cleanup()
	ctx := context.Background()
	...
	etcdMutex := etcdconcurrency.NewMutex(etcdSession, "/pebble/test/etcd-compat")
...
func TestPebble_MixedLockUsage(t *testing.T) {
	cli, cleanup := startPebbleLockTestServer(t)
	defer cleanup()
	ctx := context.Background()
	...
	customMutex := concurrency.NewMutex(customSession, "/pebble/test/mixed")

	etcdSession, err := etcdconcurrency.NewSession(cli, etcdconcurrency.WithTTL(30))
	require.NoError(t, err)
	defer etcdSession.Close()

	etcdMutex := etcdconcurrency.NewMutex(etcdSession, "/pebble/test/mixed")
...
// ============================================================================
// Pebble Key Format Verification
// ============================================================================

func TestPebble_MutexKeyFormat(t *testing.T) {
	cli, cleanup := startPebbleLockTestServer(t)
	defer cleanup()
	ctx := context.Background()
...
// ============================================================================
// Pebble Recovery Tests
// ============================================================================

func TestPebble_MutexRecoveryAfterSessionClose(t *testing.T) {
	cli, cleanup := startPebbleLockTestServer(t)
	defer cleanup()
	ctx := context.Background()

	prefix := "/pebble/test/recovery"
	...
	err = mutex1.Lock(ctx)
	require.NoError(t, err)
	t.Log("Pebble Session 1 acquired lock")

	session1.Close()
	t.Log("Pebble Session 1 closed")
	...
	assert.True(t, mutex2.IsOwner())
	t.Log("Pebble Session 2 acquired lock")

	mutex2.Unlock(ctx)
}

// ============================================================================
// Pebble Edge Case Tests
// ============================================================================

func TestPebble_MutexDifferentPrefixes(t *testing.T) {
	cli, cleanup := startPebbleLockTestServer(t)
	defer cleanup()
	ctx := context.Background()
	...
	mutex1 := concurrency.NewMutex(session, "/pebble/test/prefix1")
	mutex2 := concurrency.NewMutex(session, "/pebble/test/prefix2")
...
func TestPebble_MutexWaitingQueue(t *testing.T) {
	cli, cleanup := startPebbleLockTestServer(t)
	defer cleanup()
	ctx := context.Background()
	...
	for i := 0; i < numWaiters; i++ {
		...
		go func(id int) {
			...
			mutex := concurrency.NewMutex(session, "/pebble/test/queue")
	...
	t.Logf("Pebble Acquisition order: %v", order)
...
// ============================================================================
// Pebble Data Race Detection Tests
// ============================================================================

func TestPebble_MutexNoDataRace(t *testing.T) {
	cli, cleanup := startPebbleLockTestServer(t)
	defer cleanup()
	...
	mutex := concurrency.NewMutex(session, "/pebble/test/race")
