...
// etcdRocksDBCluster represents a cluster of etcd-compatible RocksDB nodes for testing
type etcdRocksDBCluster struct {
	peers              []string
...
func newEtcdRocksDBCluster(t *testing.T, n int) *etcdRocksDBCluster {
	...
	for i := range clus.peers {
		// Clean up data directory (raft.NewNode uses "data/rocksdb/{id}" path)
		os.RemoveAll(fmt.Sprintf("data/rocksdb/%d", i+1))

		// Create directory for RocksDB
		dbPath := fmt.Sprintf("data/rocksdb/%d/kv", i+1)
...
func (clus *etcdRocksDBCluster) Close(t *testing.T) {
	t.Log("closing etcd rocksdb cluster...")
	...
	// Clean data (raft.NewNode uses "data/rocksdb/{id}" path)
	for i := range clus.peers {
		os.RemoveAll(fmt.Sprintf("data/rocksdb/%d", i+1))
	}

	t.Log("closing etcd rocksdb cluster [done]")
}

// TestEtcdRocksDBSingleNodeOperations tests basic single-node operations
func TestEtcdRocksDBSingleNodeOperations(t *testing.T) {
	clus := newEtcdRocksDBCluster(t, 1)
	defer clus.Close(t)
...
// TestEtcdRocksDBClusterBasicConsistency tests basic cluster consistency
func TestEtcdRocksDBClusterBasicConsistency(t *testing.T) {
	const numNodes = 3
...
// TestEtcdRocksDBClusterConcurrentOperations tests concurrent operations
func TestEtcdRocksDBClusterConcurrentOperations(t *testing.T) {
	const numNodes = 3
	clus := newEtcdRocksDBCluster(t, numNodes)
	defer clus.Close(t)
...
// TestEtcdRocksDBClusterUpdateOperations tests update operations on same key
func TestEtcdRocksDBClusterUpdateOperations(t *testing.T) {
	const numNodes = 3
	clus := newEtcdRocksDBCluster(t, numNodes)
	defer clus.Close(t)
